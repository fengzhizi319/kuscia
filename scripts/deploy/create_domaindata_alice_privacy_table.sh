...
# 脚本功能：在Kuscia系统中创建Alice方的隐私组件测试DomainData资源对象
# 该脚本会根据模板文件创建一个指定域ID的Alice隐私表数据定义，并将其应用到Kubernetes集群中
# 隐私表包含 PII 字段（name/id_card/mobile）、准标识符（age/zipcode）、
# 敏感属性（diagnosis）和数值列（salary/has_disease），用于测试
...
# 读取domaindata_alice_privacy_table.yaml模板文件并进行变量替换
DOMAIN_DATASOURCE_TEMPLATE=$(sed "s/{{.DOMAIN_ID}}/${DOMAIN_ID}/g;" \
  < "${ROOT}/scripts/templates/domaindata_alice_privacy_table.yaml")
