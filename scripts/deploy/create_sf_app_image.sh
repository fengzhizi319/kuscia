...
#
# 1. 让“应用镜像”成为集群资源
#    如果只是把镜像名写在脚本里，那么它只是一个字符串。
#    放进 AppImage CRD 后，Kuscia 就能把它当成一个正式资源管理：
#      - 能 kubectl apply
#      - 能 kubectl get appimage
#      - 能版本化、更新、删除
#      - 能在多个任务之间复用
#    这就像：
#      - 镜像本体是“原材料”
...
#
# 2. 让调度器/控制器知道怎么启动它
#    Kuscia 不只是需要知道“用哪个镜像”，还需要知道：
#      - 入口命令是什么
#      - 端口怎么暴露
#      - 需要挂载哪些卷
#      - 需要哪些任务配置
#      - 是否有镜像 ID、特殊启动参数等
#    这些信息必须和镜像绑定在一起，否则每次提交任务都要手写一遍，容易出错，也不利于自动化。
#    所以 CRD 里保存的是运行契约，而不是单纯文档。
#
# 3. 支持声明式、可校验、可审计
#    放到 CRD 后，Kuscia 可以：
#      - 在资源创建时做合法性校验
#      - 统一走 Kubernetes 风格的声明式配置
#      - 让集群里所有可运行应用有一致的定义方式
#      - 便于审计、排障、回溯
#    这比把启动信息散落在脚本、环境变量、CI 配置里更稳定。
#
# 4. 让任务通过引用运行，而不是硬编码运行逻辑
#    注册后，KusciaJob 可以通过 appImageRef 引用它来启动任务 Pod。
#    这说明 AppImage 的定位是中间层抽象：
#      - 用户提交任务
#      - 任务引用一个已注册的 AppImage
#      - 控制器根据这个定义去拉起容器
#    这样任务定义和镜像启动细节就解耦了。
#
# 一句话理解
#    如果说“说明书”是给人看的，那 AppImage CRD 里的内容就是给控制器执行的合同。
#    它的价值不在“让人读”，而在于：
#      - 标准化
#      - 可复用
#      - 可校验
